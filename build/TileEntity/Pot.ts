...
namespace Sakura{
    export class Pot implements Machine{
        ...
        static tileEntityIdle:TileEntityPrototype = {
            defaultValues:{
                hasPot:false,
                burn:0,
                progress:0,
            ...
            placePot:function(id:number,count:number,data:number){
                if(!this.data.hasPot && World.getBlock(this.x,this.y + 1,this.z).id <= 0){
                    if(id === ItemID[Main.MODID + ".cooking_pot"]){
                        World.setBlock(this.x,this.y + 1,this.z,BlockID[Main.MODID + ".pot"],0);
            ...
            destroyPot:function(){
                if(this.data.hasPot && World.getBlock(this.x,this.y + 1,this.z).id === BlockID[Main.MODID + ".pot"]){
                    World.destroyBlock(this.x,this.y + 1,this.z,false);
                    World.drop(this.x,this.y,this.z,ItemID[Main.MODID + ".cooking_pot"],1,0);
                    BlockRenderer.unmapAtCoords(this.x,this.y + 1,this.z);
            ...
            startBurn:function(){
                ...
                World.setBlock(this.x,this.y,this.z,BlockID[Main.MODID + ".campfire_lit"],0);
                this.showLiquid(this.data.renderLiquid,this.data.renderLiquidAmount);
            },

            tick:function(){
                if(World.getBlock(this.x,this.y,this.z).id !== Pot.blockId && World.getBlock(this.x,this.y,this.z).id !== BlockID[Main.MODID + ".campfire_lit"]){
                    this.selfDestroy();
                }
                if(this.data.hasPot && World.getBlock(this.x,this.y + 1,this.z).id <= 0){
                    World.setBlock(this.x,this.y + 1,this.z,BlockID[Main.MODID + ".pot"],0);
        ...
        static tileEntityLit:TileEntityPrototype = {
            _initialized:true
        };
        ...
        constructor(){
            Pot.blockId = BlockID[Main.MODID + ".campfire_idle"];
            TileEntity.registerPrototype(Pot.blockId,Pot.tileEntityIdle);
            TileEntity.registerPrototype(BlockID[Main.MODID + ".campfire_lit"],Pot.tileEntityLit);
            let propertyUI = {
                standart: {
                    header: {text: {text: Translation.translate(Main.itemResource.getLang("en_us")["item." + Main.MODID + ".cooking_pot" + ".name"])}},
                    inventory: {standart: true},
                    background: {standart: true}
                },
                
                drawing: [
                    {type: "bitmap", x: 680, y: 180, bitmap: "furnace_bar_background", scale: Main.GUI_SCALE},
                    {type: "bitmap", x: 690, y: 260, bitmap: "fire_background", scale: Main.GUI_SCALE},
                    {type: "bitmap", x: 650, y: 80, bitmap: "liquid", scale: Main.GUI_SCALE,}
                ],
            ...
            let elements : any = {
                "input1" : {type: "slot", x: 480, y: 100, size: Main.GUI_SCALE * 20},
                "output" : {type: "slot", x: 760, y: 200, size: Main.GUI_SCALE * 30,onClick: function(position:BlockPosition, container:UI.Container, tileEntity:TileEntity) {//如果定义，就设定短按事件
                ...
                "liquidScale" : {type: "scale", x: 660, y: 90, direction: 0, value: 0, bitmap: "liquid2", scale: Main.GUI_SCALE},
                "progressScale": {type: "scale", x: 680, y: 180, direction: 0, value: 0.5, bitmap: "furnace_bar_scale", scale: Main.GUI_SCALE},
		        "burningScale": {type: "scale", x: 690, y: 260, direction: 1, value: 0.5, bitmap: "fire_scale", scale: Main.GUI_SCALE},
            };

            for(let i = 0;i < 8;i ++){
                elements["input2_" + i] = {type: "slot", x: 360 + (i%4) * 80, y: 180 + Math.floor(i/4) * 80, size: Main.GUI_SCALE * 20};
            }
